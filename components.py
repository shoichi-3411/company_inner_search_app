...
def display_select_mode():
    """
    回答モードのラジオボタンを表示（メイン部分用 - 廃止予定）
    """
    # この関数は互換性のために残しているが、実際の処理はdisplay_sidebar()で行う
    pass
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        # 警告メッセージを表示
        st.warning(ct.WARNING_MESSAGE, icon=ct.WARNING_ICON)


def format_file_path_with_page(file_path, page_number=None):
    """
    ファイルパスにページ番号を付加して表示用にフォーマット
    
    Args:
        file_path: ファイルパス
        page_number: ページ番号（PDFのみ）
        
    Returns:
        フォーマットされた表示用文字列
    """
    if page_number is not None and file_path.lower().endswith('.pdf'):
        return f"{file_path} (ページ {page_number})"
    else:
        return file_path
            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        # ページ番号を含むファイル情報をフォーマット
                        main_page_number = message["content"].get("main_page_number")
                        formatted_main_file = format_file_path_with_page(
                            message['content']['main_file_path'], 
                            main_page_number
                        )
                        st.success(formatted_main_file, icon=icon)
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                formatted_sub_file = format_file_path_with_page(
                                    sub_choice['source'], 
                                    sub_page_number
                                )
                                st.info(formatted_sub_file, icon=icon)
                ...
                # 「社内問い合わせ」の場合の表示処理
                    ...
                    if "file_info_list" in message["content"]:
                        ...
                        for file_info in message["content"]["file_info_list"]:
                            ...
                            # ページ番号を含むファイル情報をフォーマット
                            formatted_file = format_file_path_with_page(
                                file_info["file_path"], 
                                file_info.get("page_number")
                            )
                            st.info(formatted_file, icon=icon)
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        # ページ番号を含むファイル情報をフォーマットして表示
        formatted_main_file = format_file_path_with_page(main_file_path, main_page_number)
        st.success(formatted_main_file, icon=icon)
...
def display_contact_llm_response(llm_response):
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            # ページ番号が取得できた場合のみ、ページ番号を取得（ドキュメントによっては取得できない場合がある）
            page_number = None
            if "page" in document.metadata:
            ...
            # ページ番号を含むファイル情報をフォーマットして表示
            formatted_file = format_file_path_with_page(file_path, page_number)
            st.info(formatted_file, icon=icon)

            # 重複チェック用に、ファイルパスをリストに順次追加
            file_path_list.append(file_path)
            # ファイル情報をリストに順次追加（辞書形式で格納）
    ...
    # - 「answer」: LLMからの回答
    # - 「message」: 補足メッセージ
    # - 「file_info_list」: ファイル情報の一覧リスト（ファイルパスとページ番号を含む）
    content = {}
